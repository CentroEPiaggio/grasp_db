...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if(SPECULARIZE)
  {
    ...
    std::vector<uint> specularized_grasps = {1, 5};
    // notice that names are equal, apart from the sign of y!!!
    std::vector<std::string> specularized_grasp_names = {"hanlde_bottom (specular in xz-plane)", "hanlde_top_x- (specular in xz-plane)"};
