...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if(SPECULARIZE)
  {
    ...
    std::vector<uint> specularized_grasps = {3,4,5};
    // notice that names are equal, apart from the sign of y!!!
    std::vector<std::string> specularized_grasp_names = {"x+y+_top","x+y-_top","y-_top"};
    ...
    correspondences["x+y-_top"] = {"x+y+_top"};
    correspondences["x+y+_top"] = {"x+y-_top"};
    correspondences["y+_top"] = {"y-_top"};
