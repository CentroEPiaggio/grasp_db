...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if(SPECULARIZE)
  {
    ...
    std::vector<uint> specularized_grasps = {1,2,3,4,5};
    // notice that names are equal, apart from the sign of y!!!
    std::vector<std::string> specularized_grasp_names = {"spec_handle_side","spec_handle_side_2","spec_x-_top","y+_top","z-y_bottom_"};
    ...
    std::vector<std::string> prefixes({"bottom","y-","x-","z-"});
    std::map<std::string,std::vector<std::string>> correspondences;
    correspondences["bottom"] = {"x-","handle","y-","spec",};
    correspondences["x-"] = {"z-","handle"};
    correspondences["y-"] = {"y+","handle"};
    correspondences["z-"] = {"spec","handle","y-","y+"};
  ...
  if(!SPECULARIZE)
  {
    ...
    std::map<std::string,std::vector<std::string>> correspondences;
    correspondences["bottom"] = {"handle","x-_top","y-_top"};
